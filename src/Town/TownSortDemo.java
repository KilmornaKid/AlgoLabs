...
public class TownSortDemo {
    public static void main(String[] args) {
        ...
        Town searchTown = new Town(searchName," ",0);
        int result = Arrays.binarySearch(towns, searchTown);
        if(result < 0){
            JOptionPane.showMessageDialog(null,searchTown + " not found!");
        }else {
            String message = towns[result].getCounty()+" "+towns[result].getPopulation();
